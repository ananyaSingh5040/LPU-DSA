...
def reveal_tile(r, c, grid, labels, popup):
    if grid[r][c] == 'M':
        labels[r][c].config(bg='red', text='💣', font=("Arial", 14, "bold"))  # Mine
        game_over_popup(popup)
    else:
        labels[r][c].config(bg='green', text='💎', font=("Arial", 14, "bold"))  # Gem

def game_over_popup(popup):
    game_over = tk.Toplevel(popup)
    game_over.title("Game Over")
    tk.Label(game_over, text="You hit a mine! Game Over!", font=("Arial", 16, "bold"), fg="red").pack(padx=20, pady=20)
    tk.Button(game_over, text="Close", command=popup.destroy, font=("Arial", 12)).pack()
...
def show_game_popup(grid, path):
    ...
    for r in range(size):
        ...
        for c in range(size):
            ...
            if (r, c) in path:
                color = "blue"
            label = tk.Label(popup, bg=color, width=4, height=2, borderwidth=1, relief="solid", font=("Arial", 12))
...
def play_game(solve_with_bfs=True):
    size = int(input("Enter grid size (e.g., 5 for 5x5): "))  # User selects grid size
    num_mines = int(input(f"Enter number of mines (max {size * size - 1}): "))  # User selects number of mines
    if num_mines >= size * size:
        print("Too many mines! Adjusting to maximum allowed.")
